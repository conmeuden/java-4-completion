		...
		if (login.trim().equals("user")) {
			try {
				...
				if (user != null) {
					...
					Message message = new Message("Thông tin không hợp lệ! Thử lại!!", "error", "alert-danger");
			...
			try {
				...
				if (admin != null) {
					...
					Message message = new Message("Chi tiết không hợp lệ! Thử lại!!", "error", "alert-danger");
					session.setAttribute("message", message);
