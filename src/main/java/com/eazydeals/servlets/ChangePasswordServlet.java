	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (referrer.contains("forgot_password")) {
			...
			if (list.contains(email)) {
				...
				Message message = new Message("Chúng tôi đã gửi mã đặt lại mật khẩu đến " + email, "success",
						"alert-success");
				session.setAttribute("message", message);
				response.sendRedirect("otp_code.jsp");
			} else {
				Message message = new Message("Không tìm thấy email! Hãy thử với một email khác!", "error",
						"alert-danger");
				session.setAttribute("message", message);
			...
			Message message = new Message("Đã cập nhật mật khẩu thành công!", "error", "alert-success");
			session.setAttribute("message", message);
